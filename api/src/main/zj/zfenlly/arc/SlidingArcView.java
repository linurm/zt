...
public class SlidingArcView extends ViewGroup {
    ...
    private int viewTopChange = ScreenUtils.dp2px(800f);//view往上偏移的位置
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (chooseView != null) {
            canvas.drawText(chooseView.title, CentX, CentY, paint);
        }
        for (SignView view : views) {
            view.flush();
        }
    ...
    private void drawBg(Canvas canvas) {
        //得到原bitmap
        //把bitmap缩小为和View大小一致
        Bitmap newBitmp = Bitmap.createScaledBitmap(bgBitmap, ScreenUtils.getScreenW(), ScreenUtils.getScreenH(), false);
