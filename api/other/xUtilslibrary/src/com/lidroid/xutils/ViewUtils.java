...
public class ViewUtils {
    ...
    private static void injectObject(Object handler, ViewFinder finder) {
        ...
        if (VIEW_DEBUG)
            print(handler.toString() + ".getClass:" + handlerType.toString());
        // inject ContentView
        ...
        if (contentView != null) {
            try {
                ...
                if (VIEW_DEBUG)
                    print(handlerType.toString() + ".getAnnotation:" + contentView.toString() + ".setContentView()");
            } catch (Throwable e) {
        ...
        if (fields != null && fields.length > 0) {
            ...
            for (Field field : fields) {
                ...
                if (viewInject != null) {
                    if (VIEW_DEBUG)
                        print(field.toString() + ".getAnnotation:" + viewInject.toString());
                    try {
                        View view = finder.findViewById(viewInject.value(),
                                viewInject.parentId());
                        if (view != null) {
                            if (VIEW_DEBUG)
                                print("findViewById:" + viewInject.value());
                            field.setAccessible(true);
                            if (VIEW_DEBUG)
                                print(field.toString() + ".set(" + handler + "," + view + ")");
                    ...
                    if (resInject != null) {
                        if (VIEW_DEBUG)
                            print(field.toString() + ".getAnnotation:" + resInject.toString());
                        try {
                            Object res = ResLoader.loadRes(resInject.type(),
                                    finder.getContext(), resInject.id());
                            if (res != null) {
                                if (VIEW_DEBUG)
                                    print(":---loadRes:" + res.toString());
                                field.setAccessible(true);
                        ...
                        if (preferenceInject != null) {
                            if (VIEW_DEBUG)
                                print(field.toString() + ".getAnnotation:" + preferenceInject.toString());
                            try {
                                ...
                                if (preference != null) {
                                    if (VIEW_DEBUG)
                                        print("findPreference:" + preference.toString());
                                    field.setAccessible(true);
        ...
        if (methods != null && methods.length > 0) {
            for (Method method : methods) {
                ...
                if (VIEW_DEBUG)
                    print("getDeclaredMethod:" + method.toString());
                if (annotations != null && annotations.length > 0) {
                    for (Annotation annotation : annotations) {
                        Class<?> annType = annotation.annotationType();
                        if (VIEW_DEBUG)
                            print("getDeclaredAnnotation:" + annotation.toString());
                        if (annType.getAnnotation(EventBase.class) != null) {
                            ...
                            try {
                                ...
                                if (VIEW_DEBUG)
                                    print("getDeclaredMethod:" + valueMethod.toString());
